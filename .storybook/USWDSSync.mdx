...
# This runs:
# 1. npm run uswds:check-updates    → Check for new USWDS version
# 2. npm run uswds:update-classes   → Extract and update class list
# 3. npm run uswds:compliance       → Validate all components
# 4. npm run test:accessibility     → Run accessibility tests
```
...
npm run uswds:update-classes --check-only
...
**When It Runs:**
- Part of `npm run uswds:sync` workflow
- Can run standalone with `npm run uswds:check-updates`
...
**Easy Updates:**
- Update USWDS: `npm install @uswds/uswds@latest`
- Sync classes: `npm run uswds:sync`
...
### Manual (Optional)
- **Force Update** - If class list corrupted: `npm run uswds:update-classes --force`
- **Full Sync** - After major USWDS update: `npm run uswds:sync`
