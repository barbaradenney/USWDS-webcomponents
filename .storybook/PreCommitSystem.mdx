...
MODIFIED_COMPONENTS="language-selector" pnpm run validate:story-styles
...
**Error:**
```
❌ Component packages/uswds-wc-actions/src/components/button/usa-button.ts has no tests!
```

**Fix:**
```bash
# Create test file
touch packages/uswds-wc-actions/src/components/button/usa-button.test.ts
...
pnpm run validate:fix  # Auto-fix what's possible
pnpm run validate      # Re-run validation
```
